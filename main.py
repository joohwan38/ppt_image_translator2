...
import atexit # For ensuring cleanup

# 프로젝트 루트의 다른 .py 파일들 import
from translator import OllamaTranslator
from pptx_handler import PptxHandler
from ocr_handler import PaddleOcrHandler # PaddleOcrHandler import 확인
...
# Application 클래스에서 self.general_file_handler로 관리하도록 변경
# GENERAL_LOG_FILENAME = os.path.join(LOGS_DIR, "app_general.log")
# general_file_handler = None # 아래 Application 클래스 __init__에서 초기화
...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        self.general_file_handler = None # 파일 핸들러 인스턴스 변수로 추가

        self._setup_logging_file_handler() # 파일 핸들러 설정 메서드 호출
        ...
        self.ocr_handler = None # PaddleOcrHandler 인스턴스
        ...
        atexit.register(self.on_closing) # 프로그램 종료 시 on_closing 호출 보장

        logger.info(f"--- {APP_NAME} 시작됨 (일반 로그 파일: {self.general_file_handler.baseFilename if self.general_file_handler else '미설정'}) ---")
    ...
    def _setup_logging_file_handler(self):
        """일반 앱 로그 파일 핸들러를 설정합니다."""
        if self.general_file_handler: # 이미 설정된 경우 중복 방지
            return
        try:
            general_log_filename = os.path.join(LOGS_DIR, "app_general.log")
    ...
    def on_closing(self):
        ...
        if not self.stop_event.is_set(): # 중복 호출 방지
            ...
            if self.ocr_handler and hasattr(self.ocr_handler, 'ocr') and self.ocr_handler.ocr:
                ...
                try:
                    ...
                    # PaddleOCR에 명시적인 shutdown()이나 close() 메서드가 있다면 여기서 호출
                    # 예: if hasattr(self.ocr_handler, 'shutdown'): self.ocr_handler.shutdown()
            ...
            if self.general_file_handler:
                ...
                try:
                    ...
                    root_logger.removeHandler(self.general_file_handler) # 핸들러 제거
                    self.general_file_handler = None # 참조 제거
        ...
        if hasattr(self, 'master') and self.master.winfo_exists():
             # messagebox는 Tkinter 상태에 의존하므로, 안전한 시점에만 호출
            if messagebox.askokcancel("종료 확인", f"{APP_NAME}을(를) 종료하시겠습니까?"):
                 logger.info("모든 정리 작업 완료. 애플리케이션을 종료합니다.")
                 self.master.destroy()
            else:
                 logger.info("애플리케이션 종료 취소됨.")
                 self.stop_event.clear() # 종료 취소 시 스레드 중지 신호 해제 (필요시)
                 return # 종료 취소 시 더 이상 진행하지 않음
        else: # master가 없거나 파괴된 경우 (atexit 등으로 호출될 때)
            logger.info("애플리케이션 윈도우가 이미 없으므로 바로 종료합니다.")


    def initial_checks(self):
        logger.debug("초기 점검 시작: PaddleOCR 설치 여부 및 Ollama 상태 확인")
        # 초기에는 PaddleOCR 객체를 로드하지 않고, 설치 여부만 utils를 통해 확인 가능
        # self.check_paddleocr_status_manual(initial_check=True) # -> 번역 시작 시점으로 이동
        utils.check_paddleocr() # utils.check_paddleocr()는 설치 여부만 로깅 (객체 생성 X)
        self.ollama_status_label.config(text=f"PaddleOCR 설치: {'설치됨' if utils.check_paddleocr() else '미설치 또는 확인 불가'}")
    ...
    def create_widgets(self):
        ...
        # PaddleOCR 상태 표시는 유지하되, '확인' 버튼은 제거하거나 다른 용도로 변경 가능
        ...
        # ttk.Button(server_status_frame, text="PaddleOCR 확인", command=self.check_paddleocr_status_manual).grid(row=1, column=1, columnspan=3, padx=5, pady=2, sticky=tk.W) # 버튼 제거
        ...
        self.image_elements_label = ttk.Label(file_info_frame, text="총 이미지 수: "); self.image_elements_label.pack(anchor=tk.W, pady=1) # "이미지 내 텍스트 수"가 아닌 "총 이미지 수"
        self.total_elements_label = ttk.Label(file_info_frame, text="총 번역 시도 요소 수: "); self.total_elements_label.pack(anchor=tk.W, pady=1) # "총 번역 요소 수" -> "총 번역 시도 요소 수" (이미지는 OCR 시도 대상)
        ...
        # self.src_lang_combo.bind("<<ComboboxSelected>>", self.on_source_language_change) # 언어 변경 시 OCR 점검 제거
        ...
        log_tab_frame = ttk.Frame(right_panel_notebook, padding=5); right_panel_notebook.add(log_tab_frame, text="실행 로그")
        ...
        # ... (create_widgets의 나머지 부분은 이전과 거의 동일) ...
    ...
    def on_source_language_change(self, event=None):
        ...
        # self.check_paddleocr_status_manual(initial_check=False) # <--- 이 줄 제거
    ...
    def load_file_info(self, file_path):
        info = {"slide_count": 0, "text_elements": 0, "image_elements": 0} # image_elements_with_text 제거
        try:
            logger.debug(f"파일 정보 분석 중 (OCR 미수행): {file_path}")
            file_name = os.path.basename(file_path)
            # pptx_handler.get_file_info는 이제 OCR 핸들러를 받지 않거나, 받아도 사용하지 않음 (카운팅 목적)
            info = self.pptx_handler.get_file_info(file_path) # ocr_handler 인자 제거
            ...
            # 총 번역 시도 요소 수는 텍스트 요소와 모든 이미지 요소의 합 (이미지는 번역 시 OCR 시도 대상)
            ...
            self.total_elements_label.config(text="총 번역 시도 요소 수: -")
        ...
        if DEFAULT_MODEL not in current_models:
            ...
            if initial_check_from_ollama or messagebox.askyesno("기본 모델 다운로드", f"기본 번역 모델 '{DEFAULT_MODEL}'이(가) 없습니다. 지금 다운로드하시겠습니까? (시간 소요)"):
                ...
                self.model_download_thread = threading.Thread(
                    target=self._model_download_worker,
                    args=(DEFAULT_MODEL, self.stop_event), # stop_event 전달
                    daemon=True
                )
    ...
    def _model_download_worker(self, model_name, stop_event_ref): # stop_event_ref 매개변수 추가
        # ollama_service.pull_model_with_progress에 stop_event 전달
        success = self.ollama_service.pull_model_with_progress(
            model_name,
            self.update_model_download_progress,
            stop_event=stop_event_ref # stop_event 전달
        )
        if hasattr(self, 'master') and self.master.winfo_exists():
    ...
    def check_paddleocr_status_manual(self, initial_check=False, is_called_from_start_translation=False):
        """
        PaddleOCR 상태를 확인하고 필요시 초기화합니다.
        initial_check: 앱 시작 시 호출되는지 여부
        is_called_from_start_translation: 번역 시작 시 호출되는지 여부 (이 경우 실패 시 사용자에게 알리고 False 반환)
        반환값: PaddleOCR 준비 완료 시 True, 그렇지 않으면 False
        """
        logger.info(f"PaddleOCR 상태 확인 중... (initial: {initial_check}, from_start_translation: {is_called_from_start_translation})")
        selected_ui_lang = self.src_lang_var.get() # 번역 '대상' 언어가 아닌 '원본' 언어 기준
        paddle_ocr_code_to_use = UI_LANG_TO_PADDLEOCR_CODE.get(selected_ui_lang, DEFAULT_PADDLE_OCR_LANG)
        ...
        # 1. PaddleOCR 라이브러리 설치 여부 확인 (utils 사용)
        if not utils.check_paddleocr(): # 설치 자체가 안된 경우
            ...
            critical_fail_message = ("PaddleOCR 자동 설치 실패. 수동으로 설치해주세요.\n"
                                     ...
                                     "그 후 프로그램을 재시작해주세요.")
            should_prompt_install = not initial_check # 초기 실행 시에는 프롬프트 없이 시도
            
            if initial_check: # 앱 시작 시 자동 설치 시도
                logger.info("최초 실행: PaddleOCR 자동 설치를 시도합니다.")
                if utils.install_paddleocr():
                    logger.info("PaddleOCR 자동 설치 성공. 상태를 다시 확인합니다.")
                    if hasattr(self, 'master') and self.master.winfo_exists():
                        # 재귀 호출 대신, 성공 플래그를 반환하거나 UI를 통해 사용자에게 알림
                        self.paddleocr_status_label.config(text="PaddleOCR 상태: 설치됨 (재확인 필요)")
                        messagebox.showinfo("PaddleOCR", "PaddleOCR이 설치되었습니다. 다시 시도해주세요.")
                    return False # 설치 후 바로 사용 불가, 재확인 필요
                else:
                    logger.critical("PaddleOCR 자동 설치 실패 (최초 실행). 수동 설치 필요.")
                    if not is_called_from_start_translation: # 번역 시작 시에는 에러로 중단
                         messagebox.showerror("PaddleOCR 설치 실패", critical_fail_message)
                    return False
            elif should_prompt_install and messagebox.askyesno("PaddleOCR 설치 필요", install_prompt_message):
                if utils.install_paddleocr():
                    logger.info("PaddleOCR 자동 설치 성공. 상태를 다시 확인합니다.")
                    self.paddleocr_status_label.config(text="PaddleOCR 상태: 설치됨 (재확인 필요)")
                    messagebox.showinfo("PaddleOCR", "PaddleOCR이 설치되었습니다. 다시 시도해주세요.")
                    ...
                    if not is_called_from_start_translation:
                        messagebox.showwarning("PaddleOCR 설치 실패", critical_fail_message)
                    return False
            else: # 설치 안 함 선택 또는 프롬프트 조건 아님
                if is_called_from_start_translation:
                    messagebox.showerror("PaddleOCR 오류", "PaddleOCR이 설치되어 있지 않아 이미지 번역을 진행할 수 없습니다.")
                return False # 준비 안 됨

        # 2. PaddleOCR 라이브러리가 설치된 경우, 핸들러 객체 초기화/재초기화
        try:
            # 핸들러가 없거나, 언어가 변경되었거나, 디버그 모드가 변경된 경우 재초기화
            if not self.ocr_handler or \
               (self.ocr_handler and self.ocr_handler.current_lang != paddle_ocr_code_to_use) or \
               (self.ocr_handler and self.ocr_handler.debug_mode != debug_mode):

                log_msg_init = f"PaddleOCR 핸들러 (재)초기화 시도 (요청 언어: {paddle_ocr_code_to_use}, 현재: {self.ocr_handler.current_lang if self.ocr_handler else 'N/A'})."
                logger.info(log_msg_init)

                if self.ocr_handler and hasattr(self.ocr_handler, 'ocr') and self.ocr_handler.ocr: # 기존 핸들러 자원 해제
                    logger.debug("기존 PaddleOCR 객체 자원 해제 시도 (del self.ocr_handler.ocr)")
                    try:
                        del self.ocr_handler.ocr
                    except Exception as e_del_old_ocr:
                        logger.warning(f"기존 PaddleOCR 객체 해제 중 오류: {e_del_old_ocr}")
                    self.ocr_handler.ocr = None # 명시적으로 None 설정

                self.ocr_handler = PaddleOcrHandler(lang=paddle_ocr_code_to_use, debug_enabled=debug_mode)
                # PaddleOcrHandler 생성자에서 오류 발생 시 RuntimeError 발생

            self.paddleocr_status_label.config(text=f"PaddleOCR 상태: 준비됨 ({self.ocr_handler.current_lang})")
            logger.info(f"PaddleOCR 준비 완료 (언어: {self.ocr_handler.current_lang}).")
            return True # 준비 완료

        except RuntimeError as re_ocr: # PaddleOcrHandler 초기화 시 발생
            logger.error(f"PaddleOCR 초기화 실패 (런타임 오류 - 요청 언어: {paddle_ocr_code_to_use}): {re_ocr}", exc_info=True)
            self.paddleocr_status_label.config(text=f"PaddleOCR 상태: 초기화 실패 ({paddle_ocr_code_to_use})")
            if is_called_from_start_translation or not initial_check: # 번역 시작 시 또는 사용자의 명시적 확인 시
                messagebox.showerror("PaddleOCR 오류", f"PaddleOCR 초기화 중 오류가 발생했습니다 ({paddle_ocr_code_to_use}):\n{re_ocr}\n\n지원되지 않는 언어이거나 모델 파일 문제일 수 있습니다. 다른 원본 언어를 선택하거나 프로그램을 재시작해보세요.")
            self.ocr_handler = None
            return False
        except Exception as e_ocr_other: # 기타 예외
            logger.error(f"PaddleOCR 핸들러 생성 중 예기치 않은 오류 (요청 언어: {paddle_ocr_code_to_use}): {e_ocr_other}", exc_info=True)
            self.paddleocr_status_label.config(text="PaddleOCR 상태: 알 수 없는 오류")
            if is_called_from_start_translation or not initial_check:
                messagebox.showerror("PaddleOCR 오류", f"PaddleOCR 처리 중 예기치 않은 오류가 발생했습니다 ({paddle_ocr_code_to_use}): {e_ocr_other}")
            self.ocr_handler = None
            return False
    ...
    def swap_languages(self):
        ...
        self.on_source_language_change() # 파일 정보 다시 로드 (OCR 점검은 여기서 안 함)
    ...
    def start_translation(self):
        ...
        # --- 1. PaddleOCR 상태 확인 및 초기화 (번역 시작 시점에 수행) ---
        # is_called_from_start_translation=True로 전달하여, 실패 시 사용자에게 알리고 중단
        if not self.check_paddleocr_status_manual(is_called_from_start_translation=True):
            logger.warning("PaddleOCR 준비 실패. 번역을 진행할 수 없습니다.")
            # check_paddleocr_status_manual 내부에서 이미 사용자에게 메시지 박스를 보여줌
            return # OCR 준비 안되면 중단 (이미지 번역을 위함)

        # self.ocr_handler가 None일 수 있음 (check_paddleocr_status_manual에서 설치 안함 선택 등)
        # 이 경우, 이미지 내 텍스트는 번역되지 않음. 사용자가 이를 인지해야 함.
        if not self.ocr_handler:
            logger.warning("PaddleOCR 핸들러가 초기화되지 않았습니다. 이미지 내 텍스트는 번역되지 않습니다.")
            if not messagebox.askyesno("OCR 미준비 알림",
                                     "PaddleOCR이 준비되지 않았거나 사용자가 설치를 건너뛰었습니다.\n"
    ...
    def _translation_worker(self, file_path, src_lang, tgt_lang, model, task_log_filepath):
        ...
        try:
            ...
            # get_file_info는 이제 OCR을 수행하지 않음
            info_for_translation = self.pptx_handler.get_file_info(file_path)
            text_elements = info_for_translation.get('text_elements', 0)
            image_elements = info_for_translation.get('image_elements', 0) # 모든 이미지가 OCR 시도 대상

            # 번역할 내용이 있는지 판단 (텍스트 또는 이미지가 하나라도 있어야 함)
            if text_elements == 0 and image_elements == 0:
                logger.warning("번역할 텍스트 또는 이미지 요소가 없습니다.")
                if hasattr(self, 'master') and self.master.winfo_exists() and not self.stop_event.is_set():
                     self.master.after(0, lambda: messagebox.showinfo("정보", "파일에 번역할 텍스트 또는 이미지 요소가 없습니다."))
            ...
            # 총 번역 "시도" 요소 (텍스트 + 모든 이미지)
            total_elements_to_attempt = text_elements + image_elements
            if hasattr(self, 'master') and self.master.winfo_exists():
            ...
            output_path = self.pptx_handler.translate_presentation(
                ...
                self.translator, self.ocr_handler, # self.ocr_handler는 None일 수 있음
...
if __name__ == "__main__":
    ...
    try:
        ...
        # on_closing은 atexit과 WM_DELETE_WINDOW를 통해 이미 호출되도록 설정됨
        # 여기서 app.on_closing()을 다시 호출하면 중복 실행될 수 있으므로 주의