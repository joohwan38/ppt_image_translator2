...
def should_skip_translation(text: str) -> bool:
    ...
    if not meaningful_char_pattern.search(text):
        ...
        return True
    stripped_text = text.strip(); text_len = len(stripped_text)
    if text_len == 0: return True
    meaningful_chars_count = sum(1 for char_obj in stripped_text if meaningful_char_pattern.search(char_obj))
    if text_len > 3 and (meaningful_chars_count / text_len) < MIN_MEANINGFUL_CHAR_RATIO:
        logger.info(f"번역 스킵 (의미문자 비율 낮음 {meaningful_chars_count/text_len:.2f}): '{text}'")
        return True
    return False
...
def is_ocr_text_valid(text: str) -> bool: # OCR 결과 자체의 유효성
    ...
    if text_len > 0 and (meaningful_chars_count / text_len) < MIN_MEANINGFUL_CHAR_RATIO:
        logger.info(f"OCR 유효성 스킵 (의미문자 비율 낮음 {meaningful_chars_count / text_len:.2f}): '{stripped_text}'"); return False
...
class PptxHandler:
    ...
    def get_file_info(self, file_path, ocr_handler): # ocr_handler 인자는 유지하되, 내부에서 OCR 실행 안 함
        logger.info(f"파일 정보 분석 시작 (get_file_info - OCR 카운팅 미수행): {file_path}")
        # image_elements_with_text는 OCR을 수행하지 않으므로 항상 0으로 초기화
            ...
            for slide_idx, slide in enumerate(prs.slides):
                for shape in slide.shapes:
                    ...
                    if shape.has_text_frame and shape.text_frame.text and shape.text_frame.text.strip():
                        ...
                        # OCR을 수행하여 'image_elements_with_text'를 카운트하는 로직 제거
                        # WMF/EMF 관련 로그나 blob 속성 확인 로직도 여기서는 불필요하여 제거
                    elif shape.has_table:
                        for row in shape.table.rows:
                            for cell in row.cells:
                                if cell.text_frame.text and cell.text_frame.text.strip():
                                    info["text_elements"] +=1
            logger.info(f"파일 분석 완료 (OCR 카운팅 미수행): Slides:{info['slide_count']}, Text:{info['text_elements']}, Img:{info['image_elements']}, ImgWithText:{info['image_elements_with_text']} (항상 0)")
        ...
        try:
            with open(task_log_filepath, 'a', encoding='utf-8') as f_task_log:
                ...
                try:
                    ...
                    elements_map = []
                    f_task_log.write("--- 번역 대상 요소 분석 시작 (shape_id, WMF/EMF 사전 스킵) ---\n")
                    for slide_idx, slide in enumerate(prs.slides):
                        ...
                        for shape_in_slide in slide.shapes:
                            ...
                            if shape_in_slide.has_text_frame and shape_in_slide.text_frame.text and shape_in_slide.text_frame.text.strip():
                                ...
                                try: # .image 및 .content_type 접근 자체도 방어
                                    if not (hasattr(shape_in_slide, 'image') and shape_in_slide.image and hasattr(shape_in_slide.image, 'content_type')):
                                        f_task_log.write(f"  S{slide_idx+1}-Shape '{element_name}': image 또는 content_type 속성 없음 (elem_map).\n"); continue
                                    
                                    image_content_type = shape_in_slide.image.content_type.lower()
                                    if 'wmf' in image_content_type or 'emf' in image_content_type:
                                        f_task_log.write(f"  S{slide_idx+1}-Img '{element_name}': WMF/EMF({shape_in_slide.image.content_type})는 OCR 대상에서 제외 (elem_map).\n")
                                    elif hasattr(shape_in_slide.image, 'blob'): # blob 있는지 확인 후 has_text_in_image_bytes 호출
                                        ...
                                        else: f_task_log.write(f"  S{slide_idx+1}-Img '{element_name}': OCR 사전 검사 결과 텍스트 없음/처리 불가 (elem_map).\n")
                                    else: f_task_log.write(f"  S{slide_idx+1}-Img '{element_name}': image.blob 속성 없음 (elem_map).\n")
                    ...
                    total_elements_to_translate = len(elements_map); # ... (로깅)
                    ...
                    for item_info in elements_map:
                        # ... (current_shape_obj 찾기 및 상세 정보 로깅) ...
                        if stop_event and stop_event.is_set(): break
                        ...
                        if item_type == 'text' or item_type == 'table_cell':
                            # (이전 답변의 텍스트 처리 및 **번역 스킵 로직이 적용된 부분**을 여기에 삽입)
                            # ... (텍스트/테이블 셀 처리 로직은 이전 답변의 최종본 사용) ...
                            ...
                            if text_frame and text_frame.text and text_frame.text.strip():
                                ...
                                if should_skip_translation(original_text):
                                    f_task_log.write(f"  [번역 전] \"{original_text}\" -> [번역 후] [스킵됨 - 번역 불필요]\n")
                                else:
                                    if progress_callback: progress_callback(slide_idx+1,item_type,translated_count,total_elements_to_translate,original_text)
                                    translated_text = translator.translate_text(original_text, src_lang, tgt_lang, model_name, ollama_service, is_ocr_text=False)
                                    f_task_log.write(f"  [번역 전] \"{original_text}\" -> [번역 후] \"{translated_text}\"\n")
                                    if "오류:" not in translated_text and translated_text.strip():
                                        # (이전 답변의 텍스트 스타일링 및 삽입 로직)
                                        pass 
                                    else: f_task_log.write(f"  -> 텍스트 번역 실패/빈 결과: {translated_text}\n")
                            translated_count += 1

                        elif item_type == 'image' and ocr_handler: # elements_map에 'image'로 들어왔다는 것은 WMF/EMF가 아니고 OCR 대상일 가능성이 높음
                            try:
                                from PIL import Image as PILImage
                                if not (hasattr(current_shape_obj, 'image') and current_shape_obj.image and hasattr(current_shape_obj.image, 'blob')):
                                    f_task_log.write(f"    경고: '{element_name}' image.blob 접근 불가 (처리 단계). 건너뜀.\n"); translated_count +=1; f_task_log.write("\n"); continue
                                ...
                                if ocr_results:
                                    ...
                                    for ocr_idx, (box, (text, confidence)) in enumerate(ocr_results):
                                        ...
                                        if not is_ocr_text_valid(text): f_task_log.write(f"      OCR Text [{ocr_idx+1}]: \"{text}\" -> [스킵됨 - OCR 유효X]\n"); continue
                                        if should_skip_translation(text): f_task_log.write(f"      OCR Text [{ocr_idx+1}]: \"{text}\" -> [스킵됨 - 번역 불필요]\n"); continue
                                        
                                        translated_ocr_text = translator.translate_text(text, src_lang, tgt_lang, model_name, ollama_service, is_ocr_text=True)
                                        f_task_log.write(f"      OCR Text [{ocr_idx+1}]: \"{text}\" -> \"{translated_ocr_text}\"\n")
                                        if "오류:" not in translated_ocr_text and translated_ocr_text.strip():
                                            try:
                                                # *** 색상 보존이 적용된 render_translated_text_on_image 호출 ***
                                                edited_image_pil = ocr_handler.render_translated_text_on_image(
                                                    img_pil_original, box, translated_ocr_text, # 원본 이미지를 전달하여 색상 샘플링
                            ...
                            # (AttributeError, OSError, Exception 등 예외 처리)
